...
interface ScanConfigModalProps {
  ...
  preSelectedTargetId?: number;
}

export interface ScanData {
  tool: 'subfinder' | 'dirsearch' | 'nuclei' | 'amass';
  target_id: number;
  config: Record<string, any>;
}

const ScanConfigModal: React.FC<ScanConfigModalProps> = ({ isOpen, onClose, onSubmit, preSelectedTargetId }) => {
  const { token } = useAuth();
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  const [targets, setTargets] = useState<Target[]>([]);
  const [selectedTool, setSelectedTool] = useState<'subfinder' | 'dirsearch' | 'nuclei' | 'amass'>('subfinder');
  const [selectedTargetId, setSelectedTargetId] = useState<number | null>(preSelectedTargetId || null);
  ...
  const [amassConfig, setAmassConfig] = useState({
    passive: true,
    active: false,
    brute: false,
    timeout: 30,
  ...
  // Load targets and set pre-selected target
  useEffect(() => {
    if (isOpen && token) {
      fetch('/api/v1/targets?limit=1000', {
        ...
        .then(res => res.json())
        .then(data => {
          // API returns {targets: [], total: number, page: number, size: number}
          setTargets(data.targets || []);
          // Set pre-selected target if provided
          if (preSelectedTargetId && !selectedTargetId) {
            setSelectedTargetId(preSelectedTargetId);
          }
        })
        .catch(console.error);
    }
  }, [isOpen, token, preSelectedTargetId]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{
      ...
      <div className="rounded-xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-y-auto scan-config-modal" style={{
        backgroundColor: isDark ? '#1f2937' : '#ffffff'
      }}>
        <style>{`
          .scan-config-modal input[type="text"],
          .scan-config-modal input[type="number"],
          .scan-config-modal select {
        ...
        <div className="sticky top-0 border-b flex items-center justify-between z-10" style={{
          ...
          <h2 className="text-xl font-bold" style={{ color: isDark ? '#ffffff' : '#111827' }}>Configure Scan</h2>
          <button
            onClick={onClose}
        ...
        <div style={{ padding: '18px 28px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div>
            ...
            <div className="grid grid-cols-4" style={{ gap: '12px' }}>
              {['subfinder', 'dirsearch', 'nuclei', 'amass'].map(tool => (
                <button
                  key={tool}
                  type="button"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div className="grid grid-cols-2" style={{ gap: '20px' }}>
                  <div>
                    ...
                    <input
                      type="number"
                      value={amassConfig.timeout}
                      onChange={(e) => setAmassConfig({...amassConfig, timeout: Number(e.target.value)})}
                      className="w-full border rounded-lg"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={amassConfig.max_dns_queries}
                      onChange={(e) => setAmassConfig({...amassConfig, max_dns_queries: Number(e.target.value)})}
                      className="w-full border rounded-lg"
                    />
                  </div>
                </div>
                <div className="flex items-center" style={{ gap: '32px' }}>
                  <label className="flex items-center">
                    <input
                      type="checkbox"
                      checked={amassConfig.passive}
                      onChange={(e) => setAmassConfig({...amassConfig, passive: e.target.checked})}
                      className="rounded border-gray-300 dark:border-gray-600 text-blue-600"
                    />
                    <span style={{ marginLeft: '10px', fontSize: '14px', color: isDark ? '#d1d5db' : '#111827' }}>Passive mode</span>
                  </label>
                  <label className="flex items-center">
                    <input
                      type="checkbox"
                      checked={amassConfig.active}
                      onChange={(e) => setAmassConfig({...amassConfig, active: e.target.checked})}
                      className="rounded border-gray-300 dark:border-gray-600 text-blue-600"
                    />
                    <span style={{ marginLeft: '10px', fontSize: '14px', color: isDark ? '#d1d5db' : '#111827' }}>Active mode</span>
                  </label>
                  <label className="flex items-center">
                    <input
                      type="checkbox"
                      checked={amassConfig.brute}
                      onChange={(e) => setAmassConfig({...amassConfig, brute: e.target.checked})}
                      className="rounded border-gray-300 dark:border-gray-600 text-blue-600"
                    />
                    <span style={{ marginLeft: '10px', fontSize: '14px', color: isDark ? '#d1d5db' : '#111827' }}>Brute force</span>
                  </label>
                </div>
              </div>
            )}
